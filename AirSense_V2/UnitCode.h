...
void readData(struct StructPMS *p)
{
  ...
  while (DUSTSerial.available()) { DUSTSerial.read(); }
...
void RGB_Show(const char RED_PIN, const char GREEN_PIN, const char BLUE_PIN, int Value)
{
	...
	if(Value < 12){redPWM = 0; greenPWM = 255; bluePWM = 0;} // Green (0-255-0)
	else if(Value >= 12 && Value < 35){redPWM = 255; greenPWM = 255; bluePWM = 0;} //Yellow (255-255-0)
	else if(Value >= 35 && Value < 55){redPWM = 255; greenPWM = 165; bluePWM = 0;} //Orange	(255-165-0)
	else if(Value >= 55 && Value < 150){redPWM = 255; greenPWM = 0; bluePWM = 0;} //Red	(255-0-0)
	else if(Value >= 150 && Value < 250){redPWM = 128; greenPWM = 0; bluePWM = 128;} //Purple (128-0-128)
	else{redPWM = 128; greenPWM = 255; bluePWM = 0;} //Marron (128,0,0)
